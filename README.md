...
Если нужно пропустить автоматическое создание суперюзера:

```bash
./scripts/init_deploy.sh --skip-superuser
```
...
Для ручного локального создания администратора по-прежнему можно использовать:
